...
let handler = async (m, { conn, usedPrefix, DevMode }) => { 
    try { 
        ...
        if (global.DATABASE._data.users[m.sender].healt > 79) {
            if (new Date - global.DATABASE._data.users[m.sender].lastadventure > 600000) {
            ...
            let _coin = `${Math.floor(Math.random() * 20)}`.trim()
            let coin = (_coin * 1)
            ...
            let str = `
Nyawa mu berkurang -${healt * 1} karena Kamu telah berpetualang sampai ${pickRandom(['Amerika 🇺🇸', 'Jepang 🇯🇵', 'Korea 🇰🇵', 'Amerika 🇺🇸', 'Iraq 🇮🇶', 'Arab 🇸🇦', 'Pakistan 🇵🇰', 'German 🇩🇪', 'Finlandia 🇫🇮', 'Ke bawa dunia mimpi 🌏', 'Ujung dunia 🌏', 'Bulan 🌑', 'Matahari ☀️', 'Hatinya dia 💕', '...'])} dan mendapatkan
🧬 *Exp:* ${exp} 
💵 *Uang:* ${uang}
🪙 *Coin:* *${coin}*
...
`.trim()
            ...
            global.DATABASE._data.users[m.sender].coin += coin * 1
