...
let handler = async (m, {usedPrefix}) => {
  ...
  let txt = `*Hai ${name.vnmae || name.notify || name.name || '+' + name.jid.split`@`[0]}*\n\nSilahkan pilih metode pembayaran berikut untuk melanjutkan membeli 10 diamond 💎 = Rp 5.000 💵.`;
    const buttons = [
      { buttonId: `.dn10d`, buttonText: { displayText: '💶 Dana' }, type: 1 },
      { buttonId: `.p10d`, buttonText: { displayText: '💸 Pulsa' }, type: 1 },
    ];
