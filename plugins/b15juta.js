...
let handler = async (m, {usedPrefix}) => {
  ...
  let txt = `*Hai ${name.vnmae || name.notify || name.name || '+' + name.jid.split`@`[0]}*\n\nSilahkan pilih metode pembayaran berikut untuk melanjutkan membeli 15 juta 💰 = Rp 4.000 💵.`;
    const buttons = [
      { buttonId: `.dn15juta`, buttonText: { displayText: '💶 Dana' }, type: 1 },
      { buttonId: `.p15juta`, buttonText: { displayText: '💸 Pulsa' }, type: 1 },
    ];
