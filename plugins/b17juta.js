...
let handler = async (m, {usedPrefix}) => {
  ...
  let txt = `*Hai ${name.vnmae || name.notify || name.name || '+' + name.jid.split`@`[0]}*\n\nSilahkan pilih metode pembayaran berikut untuk melanjutkan membeli 17 juta 💰 = Rp 8.000 💵.`;
    const buttons = [
      { buttonId: `.dn17juta`, buttonText: { displayText: '💶 Dana' }, type: 1 },
      { buttonId: `.p17juta`, buttonText: { displayText: '💸 Pulsa' }, type: 1 },
    ];
