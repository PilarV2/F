...
let handler = async (m, {usedPrefix}) => {
  ...
  let txt = `*Hai ${name.vnmae || name.notify || name.name || '+' + name.jid.split`@`[0]}*\n\nSilahkan pilih metode pembayaran berikut untuk melanjutkan membeli 1 miliar 💰 = Rp 15.000 💵.`;
    const buttons = [
      { buttonId: `.dn1m`, buttonText: { displayText: '💶 Dana' }, type: 1 },
      { buttonId: `.p1m`, buttonText: { displayText: '💸 Pulsa' }, type: 1 },
    ];
