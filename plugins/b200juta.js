...
let handler = async (m, {usedPrefix}) => {
  ...
  let txt = `*Hai ${name.vnmae || name.notify || name.name || '+' + name.jid.split`@`[0]}*\n\nSilahkan pilih metode pembayaran berikut untuk melanjutkan membeli 200 juta 💰 = Rp 12.000 💵.`;
    const buttons = [
      { buttonId: `.dn200juta`, buttonText: { displayText: '💶 Dana' }, type: 1 },
      { buttonId: `.p200juta`, buttonText: { displayText: '💸 Pulsa' }, type: 1 },
    ];
