...
let handler = async (m, {usedPrefix}) => {
  ...
  let txt = `*Hai ${name.vnmae || name.notify || name.name || '+' + name.jid.split`@`[0]}*\n\nSilahkan pilih metode pembayaran berikut untuk melanjutkan membeli 60 diamond 💎 = Rp 9.000 💵.`;
    const buttons = [
      { buttonId: `.dn60d`, buttonText: { displayText: '💶 Dana' }, type: 1 },
      { buttonId: `.p60d`, buttonText: { displayText: '💸 Pulsa' }, type: 1 },
    ];
