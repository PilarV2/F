...
let handler = async (m, {usedPrefix}) => {
  ...
  let txt = `*Hai ${name.vnmae || name.notify || name.name || '+' + name.jid.split`@`[0]}*\n\nSilahkan pilih metode pembayaran berikut untuk melanjutkan membeli 7 miliar 💰 = Rp 23.000 💵.`;
    const buttons = [
      { buttonId: `.dn7m`, buttonText: { displayText: '💶 Dana' }, type: 1 },
      { buttonId: `.p7m`, buttonText: { displayText: '💸 Pulsa' }, type: 1 },
    ];
