...
let handler = async (m, { conn }) => {
    let __timers = (new Date - global.DATABASE._data.users[m.sender].lastberburu)
        let _timers = (172800000 - __timers) 
        let timers = clockString(_timers)
        if (new Date - global.DATABASE.data.users[m.sender].lastberburu > 172800000) {
let randomaku1 = `${Math.floor(Math.random() * 2)}`
let randomaku2 = `${Math.floor(Math.random() * 2)}`
let randomaku4 = `${Math.floor(Math.random() * 2)}`
let randomaku3 = `${Math.floor(Math.random() * 2)}`
let randomaku5 = `${Math.floor(Math.random() * 2)}`
