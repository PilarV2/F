...
*${usedPrefix + command} start* = start game`)
  ...
  if (args[0] == "join"){
    ...
    if (global.DATABASE.data.users[m.sender].money < 10000) return m.reply("*Uang kamu minimal Rp. 10.000 untuk bermain game ini.*")
    // FIRST PLAYER
    if (!(m.chat in conn.war)) {
      conn.war2[m.chat] = {"war" : false, "turn" : 0, "time" : 0, "money" : 0}
      conn.war[m.chat] = []
      xp = global.DATABASE.data.users[m.sender].xp
      conn.war[m.chat][0] = {"user": m.sender, "hp": 5000, "lvl": global.DATABASE.data.users[m.sender].level, "turn" : false}
      for (i=1;i<10;i++){
      ...
      if (args[1]){
        if (args[1].toLowerCase() == "a"){
          ...
          for (i = 1 ; i < 5 ; i++) {
            if (conn.war[m.chat][i].user == ""){
              xp = global.DATABASE.data.users[m.sender].xp
              conn.war[m.chat][i] = {"user" : m.sender, "hp" : 5000, "lvl" : global.DATABASE.data.users[m.sender].level, "turn" : false}
              total = 0
              for (i = 0 ; i < 10 ; i++) {
                if (conn.war[m.chat][i].user == ""){
                  total += 1
