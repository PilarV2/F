...
let handler = async (m, {usedPrefix}) => {
  ...
  let txt = `*Hai ${name.vnmae || name.notify || name.name || '+' + name.jid.split`@`[0]}*\n\nSilahkan pilih metode pembayaran berikut untuk melanjutkan membeli premnya.`;
    const buttons = [
      { buttonId: `.dnprem`, buttonText: { displayText: '💶 Dana' }, type: 1 },
      { buttonId: `.pulsaprem`, buttonText: { displayText: '💸 Pulsa' }, type: 1 },
    ];
...
handler.command = /^(listpembayaranprem)$/i
