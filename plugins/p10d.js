...
let handler = async (m, {usedPrefix}) => {
  ...
  let txt = `*Halo ${name.vnmae || name.notify || name.name || '+' + name.jid.split`@`[0]}*\nSilahkan bayar untuk membeli 10 diamond 💎 = Rp 5.000 💵\nPulsa: ${global.nomortopup}`;
    const buttons = [
      { buttonId: `.topup`, buttonText: { displayText: '⬅️ Back' }, type: 1 },
      { buttonId: `.buktitopup 10 diamond 💎 = Rp 5.000 💵`, buttonText: { displayText: '📮 Klik Disini Jika Sudah Bayar' }, type: 1 },
    ];
