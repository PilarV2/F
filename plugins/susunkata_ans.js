...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.susunkata[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            DATABASE.data.users[m.sender].exp += this.susunkata[id][2]
            await this.sendButton(m.chat, `Selamat Anda Benar ✅ +${this.susunkata[id][2]} XP`, '© Nekohime', 'Susun Kata', '.susunkata', m)
