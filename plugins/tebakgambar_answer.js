...
handler.before = async function (m) {
  ...
  if (m.quoted.id == this.tebakgambar[id][0].id) {
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase()) {
      DATABASE.data.users[m.sender].exp += this.tebakgambar[id][2]
      await this.sendButton(m.chat, `Selamat Jawaban kamu Benar ✅ +${this.tebakgambar[id][2]} XP`, '© Nekohime', 'Tebak Gambar', '.tebakgambar', m)
